...
if args.single == None and args.fastq1 != None and args.fastq2 != None:
	...
	kallisto_mean=kallisto >> group_by(X.species) >> summarize(read_count = X.read_count.mean(), tpm = X.tpm.mean()) >> mask(X.read_count > 0) >> ungroup() >> mutate(read_percent=(X.read_count/X.read_count.sum())*100, tpm_percent=(X.tpm/X.tpm.sum())*100)
	kallisto_mean=kallisto_mean.sort_values("tpm_percent", ascending=False)
	kallisto_mean=kallisto_mean.round(3)
	kallisto_mean2=kallisto_mean >> mask(X.tpm_percent > 1)
	print(kallisto_mean2.to_string(index=False))
	kallisto_mean.to_csv("kallisto_contamination.tsv",sep='\t',index=False)
